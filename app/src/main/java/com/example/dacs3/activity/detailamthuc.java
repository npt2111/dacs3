...
public class detailamthuc extends AppCompatActivity {
    ImageView imageView, backpage, btn_ytAT;
    TextView titleView, locaTextView, descriptionView, scoreView;
    private RatingBar ScoreAT;
    private Button btn_dgAT;
    private EditText edtbinhluanAT;
    private DatabaseReference databaseReference;
    RecyclerView rv_dgat;
   BinhluanAdapter binhluanAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detailamthuc);
        ...
        ImageView imageView = findViewById(R.id.picImgDe);
        TextView titleView = findViewById(R.id.titleDe);
        TextView locationView = findViewById(R.id.locationDe);
        TextView descriptionView = findViewById(R.id.descriptionDe);
        scoreView = findViewById(R.id.scoreDe);
        btn_ytAT= (ImageView) findViewById(R.id.btn_ytAT);
        btn_ytAT.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                if (isAdmin || isUser){
                    ...
                    Toast.makeText(detailamthuc.this, "Đăng nhập để thực hiện!", Toast.LENGTH_SHORT).show();
                }
        ...
        ScoreAT = findViewById(R.id.ScoreAT);
        edtbinhluanAT = findViewById(R.id.edtbinhluanAT);
        btn_dgAT = findViewById(R.id.btn_dgAT);

        rv_dgat = (RecyclerView) findViewById(R.id.rv_dgat);
        rv_dgat.setLayoutManager(new LinearLayoutManager(this));

        databaseReference = FirebaseDatabase.getInstance().getReference("danhgiaamthuc");

        uploadtoBinhLuan();

        btn_dgAT.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        backpage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                boolean isUser = sharetaikhoan.getBoolean("isUser", false);
                if(isAdmin){
                    startActivity(new Intent(detailamthuc.this, AmthucMain.class));

                }else {

                    startActivity(new Intent(detailamthuc.this, AmthucUser.class));
                }
    ...
    private void skyeuthich(){
        ...
        referenceYT.child(idYT).setValue(yeuThichModel).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                ...
                Toast.makeText(detailamthuc.this, "Đã thêm vào danh sách yêu thích", Toast.LENGTH_SHORT).show();
    ...
    private void submitRating() {
        float score = ScoreAT.getRating();


        // Kiểm tra nếu rating hợp lệ
        if (score == 0) {
            Toast.makeText(detailamthuc.this, "Vui lòng chọn số sao để đánh giá", Toast.LENGTH_SHORT).show();
            return;
        }
        SharedPreferences sharetaikhoan = getSharedPreferences("LoginPrefs", MODE_PRIVATE);
        String username = sharetaikhoan.getString("username", "Guest");
        String binhluan = edtbinhluanAT.getText().toString();
        ...
        databaseReference.child(idScore).setValue(danhgia).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                ...
                Toast.makeText(detailamthuc.this, "Đánh giá thành công", Toast.LENGTH_SHORT).show();

                ScoreAT.setRating(0);
                edtbinhluanAT.setText("");
                recreate();
            } else {
                Toast.makeText(detailamthuc.this, "Đánh giá thất bại", Toast.LENGTH_SHORT).show();
            }
        });
    }
    private void uploadtoBinhLuan() {
        ...
        FirebaseRecyclerOptions<Danhgia> options =
                new FirebaseRecyclerOptions.Builder<Danhgia>()
                        .setQuery(FirebaseDatabase.getInstance().getReference().child("danhgiaamthuc").orderByChild("title").equalTo(title), Danhgia.class)
                        .build();
        binhluanAdapter = new BinhluanAdapter(options);
        rv_dgat.setAdapter(binhluanAdapter);
    }
    private void calculateAverageScore() {
        ...
        FirebaseDatabase.getInstance().getReference().child("danhgiaamthuc").orderByChild("title").equalTo(title).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (count > 0) {
                    ...
                    DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference().child("Amthuc");
