...
public class AmthucUserAdapter extends FirebaseRecyclerAdapter<Amthuc, AmthucUserAdapter.rvusHolder> {
    public AmthucUserAdapter(@NonNull FirebaseRecyclerOptions<Amthuc> options) {
        super(options);
    ...
    @Override
    protected void onBindViewHolder(@NonNull AmthucUserAdapter.rvusHolder rvusHolder, int position, @NonNull Amthuc amthuc) {



        rvusHolder.title.setText(amthuc.getTitle());
        rvusHolder.location.setText(amthuc.getLocation());
        float score = (float) amthuc.getScore();
        DecimalFormat df = new DecimalFormat("#.#");
        String formattedScore = df.format(score);
        rvusHolder.score.setText(formattedScore);
        Glide.with( rvusHolder.pic.getContext())
                .load(amthuc.getPic())
        ...
        rvusHolder.cardView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {


                Intent intCard = new Intent( rvusHolder.title.getContext(), detailamthuc.class );
                ...
                rvusHolder.title.getContext().startActivity(intCard);
            }
        });
    ...
    @NonNull
    @Override
    public AmthucUserAdapter.rvusHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_rvuser,parent,false);

        return new AmthucUserAdapter.rvusHolder(view);
    }

    class rvusHolder extends RecyclerView.ViewHolder{
        ImageView pic;
        TextView title,location,description,score;

        CardView cardView;

        public rvusHolder(@NonNull View itemView) {
            super(itemView);
